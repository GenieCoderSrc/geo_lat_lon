...
class GeoCollectionReference<T> {
  ...
  Stream<List<GeoDocumentSnapshot<T>>> subscribeWithinWithDistance({
    ...
    final filteredGeoDocumentSnapshots = mergedCollectionStreams.map((
      ...
      return filteredList.toList()..sort(
        (final a, final b) =>
  ...
  List<Future<List<QueryDocumentSnapshot<T>>>> _collectionFutures({
    ...
    return _geohashes(radiusInKm: radiusInKm, center: center).map((
      ...
      querySnapshot = await query.get(
        GetOptions(source: isCacheFirst ? Source.cache : Source.serverAndCache),
      );
  ...
  Query<T> geoQuery({
    ...
    return query.orderBy('$field.$geohashField').startAt([geohash]).endAt([
      '$geohash$_rangeQueryEndAtCharacter',
    ]);
  }
